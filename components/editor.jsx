...
export default function RichTextEditor({
    ...
    async (event) => {
      ...
      if (files?.length) {
        for (const file of files) {
          const source = await imageToBase64(file);
  ...
  return (
    <div className="form-control">
      ...
      <div className="textarea textarea-bordered p-0" disabled={disabled}>
        <ul className="menu menu-horizontal w-full rounded-t-btn bg-base-200">
          ...
          <li className={!editor?.can().undo() || disabled ? "disabled" : ""}>
            <button
              type="button"
          ...
          <li className={!editor?.can().redo() || disabled ? "disabled" : ""}>
            <button
              type="button"
